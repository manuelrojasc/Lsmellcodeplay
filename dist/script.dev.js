  ...
  $.fn.honeycombs = function (options) {
    ...
    function initialise(element) {
      ...
      function buildHtml() {
        ...
        $(element).find('.comb').each(function () {
          ...
          if ($(this).find('span.comb-title').length > 0) {
            $(this).find('.inner_span .inner-text-title').html($(this).find('span.comb-title').html());
          }

          if ($(this).find('span.comb-description-short').length > 0) {
            $(this).find('.inner_span .inner-text-description-short').html($(this).find('span.comb-description-short').html());
          }
        });
      }
      /**
      ...
      function reorder(animate) {
        ...
        $wrapper.width(cols * (combWidth / 4 * 3 + settings.margin) + combWidth / 4).innerHeight((row + 1) * (combHeight + settings.margin) + combHeight / 2);
      }
      ...
      function CheckInfoBlockOutsideViewport(el) {
        var $infoBlock = $(el).find('.inner-text-description-long');
        var left = getLeftInfoBlock($infoBlock); // Get the left position of the parent relative to the document

        var $parentLeft = $(el).parent().offset().left; // Determine the position of the right side of the bouding box of the Info Block
      ...
      function clickInfoBlockHandler(event) {
        ...
        var $dataLink = $(this).find('.inner-text-description-long').attr('data-link'); // If long description is empty and data-link attribute is available, then follow the link directly
      ...
      function addEvents() {
        ...
        $('.close').on('click', closeBtnHandler); // TOFIX: don't let javascript hijack the click event, but that it can be opened in a new tab with keyboard/mouse
...
function showModal(id) {
  var info = content.find(function (ele) {
    return ele.id == id;
  });
  var content = document.getElementById('myModalContent');
  content.innerHTML = "\n      <h4 class=\"containerTitle\">".concat(info.title, "</h4>\n      <div class=\"containerInfo\">\n        <div class=\"containerInfo-img\"><img src=\".").concat(info.icons, "\"/></div>\n        <p class=\"containerInfo-text\">").concat(info.content, "</p>\n      </div>\n    ");
